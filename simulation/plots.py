...
def read_pion_log(file):
    drc_data = []
    drc_pattern = re.compile(r'.*TRACE: (\d{2}:\d{2}:\d{2}\.\d{6}).* ts=(\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}.\d{6}), seq=(\d+), interArrivalTime=(\d+), interDepartureTime=(\d+), interGroupDelay=(-?\d+), estimate=(-?\d+.\d+), threshold=(\d+.\d+), usage=(-?\d+), state=(-?\d+)')
        ...
        for line in f:
            ...
            if match:
                drc_data.append({
                    ...
                    'threshold': float(match.group(8)),
                    'usage': int(match.group(9)),
                    'state': int(match.group(10)),
                })
