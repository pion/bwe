...
def read_pion_log(file):
    drc_data = []
    drc_pattern = re.compile(r'.*TRACE: (\d{2}:\d{2}:\d{2}\.\d{6}).* ts=(\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}.\d{6}), seq=(\d+), interArrivalTime=(-?\d+), interDepartureTime=(-?\d+), interGroupDelay=(-?\d+), estimate=(-?\d+.\d+), modifiedTrend=(-?\d+.\d+), threshold=(\d+.\d+), usage=(-?\d+), state=(-?\d+)')
        ...
        for line in f:
            ...
            if match:
                drc_data.append({
                    ...
                    'inter_arrival_time': int(match.group(4)),
                    'inter_departure_time': int(match.group(5)),
                    'inter_group_delay': int(match.group(6)),
                    'estimate': float(match.group(7)),
                    'modified_trend': float(match.group(8)),
...
def plot_gcc_inter_group_delay(ax, df):
    ax.plot(df.index, df['inter_group_delay'] * 1e-6,
...
def plot_gcc_estimates(ax, df):
    ...
    ax.plot(df.index, df['modified_trend'], label='modified_trend',
            linewidth=0.5)
...
def plot(output, json, stderr):
    ...
    fig, ax = plt.subplots(8, 1, sharex=True, figsize=(10, 12),
    ...
    plot_gcc_inter_group_delay(ax[3], gcc_drc)
    ax[3].set_title('GCC Inter Group Delay')

    plot_gcc_estimates(ax[4], gcc_drc)
    ax[4].set_title('GCC Estimates')

    plot_gcc_usage_and_state(ax[5], gcc_drc)
    ax[5].set_title('GCC Usage and State')

    plot_gcc_rtt(ax[6], gcc_sbwe)
    ax[6].set_title('GCC RTT')

    plot_loss(ax[7], loss)
    ax[7].set_title('Packet Loss')
    ax[7].yaxis.set_major_formatter(mticker.PercentFormatter(xmax=1.0))
...
def plot_all(input, output):
    json_logs = {Path(f).stem: f for f in glob.glob(f'{input}/*.jsonl')}
    stderr_logs = {Path(f).stem: f for f in glob.glob(f'{input}/*.stderr')}

    names = sorted(json_logs.keys() & stderr_logs.keys())
    for name in sorted(json_logs.keys() ^ stderr_logs.keys()):
        print(f'skipping {name}: has no matching .jsonl and .stderr pair')

    with ProcessPoolExecutor() as executor:
        list(executor.map(plot, [output] * len(names),
                          [json_logs[n] for n in names],
                          [stderr_logs[n] for n in names]))
