...
var (
	...
	// ErrInvalidDeliveryRateWindow is returned by WithDeliveryRateWindow if the
	// given window is not positive.
	ErrInvalidDeliveryRateWindow = errors.New("delivery rate window must be positive")
)

const (
	defaultDeliveryRateWindow = time.Second
	defaultRTTAlpha           = 0.1
)
...
// WithDeliveryRateWindow configures the window over which the delivered rate is
// measured.
func WithDeliveryRateWindow(window time.Duration) Option {
	return func(ssc *SendSideController) error {
		if window <= 0 {
			return ErrInvalidDeliveryRateWindow
		}
		ssc.deliveryRateWindow = window
...
type SendSideController struct {
	...
	deliveryRateWindow time.Duration
...
func NewSendSideController(initialRate, minRate, maxRate int, opts ...Option) (*SendSideController, error) {
	...
	ssc := &SendSideController{
		...
		deliveryRateWindow: defaultDeliveryRateWindow,
		lrc:                newLossRateController(initialRate, minRate, maxRate),
		rtt:                newEWMA(defaultRTTAlpha),
	...
	ssc.dre = newDeliveryRateEstimator(ssc.deliveryRateWindow)
	ssc.drc = newDelayRateController(initialRate, minRate, maxRate, ssc.logFactory.NewLogger("bwe_delay_rate_controller"))

	return ssc, nil
}

// TargetRate returns the current target rate.
func (c *SendSideController) TargetRate() int {
	return c.targetRate
}
...
// passed in ascending arrival-time order. Packets and groups that are reordered
// relative to their departure times are ignored by the arrival-time model.
...
func (c *SendSideController) OnFeedback(ts time.Time, rtt time.Duration) int {
	...
	c.lastTS = ts
	c.rtt.update(float64(rtt))
	smoothedRTT := time.Duration(c.rtt.avg())
	delivered := c.dre.getRate()
	lossTarget, _ := c.lrc.update(ts, smoothedRTT, delivered)
	delayTarget := c.drc.update(ts, delivered, smoothedRTT)
	c.targetRate = min(lossTarget, delayTarget)
	c.log.Tracef(
		"rtt=%v, delivered=%v, lossTarget=%v, delayTarget=%v, target=%v",
		smoothedRTT.Nanoseconds(),
		delivered,
		lossTarget,
		delayTarget,
		c.targetRate,
